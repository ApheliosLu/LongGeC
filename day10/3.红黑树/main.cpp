 ...
 *
 */
static void rbtree_left_rotate(RBRoot* root, Node* x)//左旋，“六条指向关系”，左旋前的134、左旋后的125
{
	// 设置x的右孩子为y
	Node* y = x->right;

	// 将 “y的左孩子” 设为 “x的右孩子”；前4后5
	...
	if (x->parent == NULL)//修改红黑树的根节点
	{
		...
		root->node = y;            // 如果 “x的父亲” 是空节点（代表x原本是树根），则将y设为根节点
	}
	else
	{
		if (x->parent->left == x)
 ...
 */
static void rbtree_insert_fixup1(RBRoot* root, Node* node)//原fixup函数
{
	Node* parent, * gparent;
...
static void rbtree_insert_fixup(RBRoot* root, Node* node)//自己实现fixup代码
{
	Node* parent;  //Node*等价于RBTree
	Node* tmp;
	//使用fixup函数的条件：若“父结点存在，并且父结点的颜色是红色”
	while ((parent = rb_parent(node)) && parent->color == RED)
	{
		...
		if (gparent->left == parent)
		{
			...
			if (uncle && rb_is_red(uncle))
			{
				...
				rb_set_black(uncle);
				rb_set_red(gparent);
				node = gparent;//上色之后，将gparent重新作为局部变量，continue上述while循环
				continue;
			}
			//情形四：叔为黑或无，父为爷左、子为父右；先左旋，再交换父子结点
			...
			//情形五：叔为黑或无，父为爷左、子为父左；先右旋，再上色
			rbtree_right_rotate(root, gparent);
			rb_set_black(parent);
			rb_set_red(gparent);//黑结点的子结点可为一边黑一边红
		}
		else//若“父结点”是“祖父节点”的“右孩子”
		{
			//情形三：叔为红结点
			Node* uncle = gparent->left;//当插入60时，调整颜色即可，调整颜色后不符合红黑树，递归进行
			if (uncle && rb_is_red(uncle))
			{
				...
				rb_set_red(gparent);
				node = gparent;
				continue;//继续进行调整；continue代表下面的不判断，重新回到while判断
			}
			//情形四，叔叔是黑色或没有，子结点是父结点的左孩子
			if (parent->left == node) 
			{
				rbtree_right_rotate(root, parent);
				tmp = parent;
			...
			//情形五:叔叔是黑色或没有，且当前结点是右孩子
			rbtree_left_rotate(root, gparent);//直接左旋
			rb_set_black(parent);//旋转前设置好颜色
			rb_set_red(gparent);
		}
	}
	//没有父亲，不使用fixup函数，直接将根结点设为黑色
	rb_set_black(root->node);
}
...
static void rbtree_insert(RBRoot* root, Node* node)
{
	...
	rb_parent(node) = y;//找到父节点并把要插入节点的父节点的指针修改；宏替换
	//修改父节点的子节点指针
	if (y != NULL)
	{
		if (node->key < y->key)

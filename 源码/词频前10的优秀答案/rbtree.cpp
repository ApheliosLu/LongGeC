...
{
	if (x == NULL || (x->key.value == key.value)&&(strcmp(x->key.key,key.key)==0))
		return x;

	if (key.value < x->key.value)
...
{
	while ((x != NULL) && (x->key.value != key.value))
	{
		if (key.value < x->key.value)
			x = x->left;
		else
...
static void rbtree_print(RBTree tree, Type key, int direction)
{
	if (tree != NULL)
	{
		if (direction == 0)    // tree是根节点
			...
			printf("%2d(%s) is %2d's %6s child\n", tree->key.value, rb_is_red(tree) ? "R" : "B", key.value, direction == 1 ? "right" : "left");
